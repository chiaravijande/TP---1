...
bool validar_nombre(const char *nombre) {
  ...
  for (size_t i = 0; i < strlen(nombre); i++) {
    ...
    if (isalpha(nombre[i]))
      contiene_letra = true; // Detecta si el nombre tiene al menos una letra
  }

  if (!contiene_letra) {
    fprintf(stderr,
            "Advertencia: Nombre inválido porque no contiene letras: %s\n",
            nombre);
    return false;
  }
...
bool leer_linea_pokemon(const char *linea, struct pokemon *p) {
  ...
  int leidos = sscanf(linea, "%u;%254[^;];%19[^;];%u;%u;%u", &p->id, nombre,
                      tipo_str, &p->fuerza, &p->destreza, &p->inteligencia);

  // Validación para líneas mal formateadas
  if (leidos != 6) {
    fprintf(stderr,
            "Advertencia: Campos incompletos o inválidos en la línea: %s\n",
            linea);
    free(nombre);
    return false;
  }
  ...
  if (strlen(nombre) >= MAX_NOMBRE) {
    fprintf(stderr,
            "Advertencia: Nombre demasiado largo (más de %d caracteres): %s\n",
            MAX_NOMBRE - 1, nombre);
    free(nombre);
    return false;
  }
  ...
  if (p->id <= 0 || p->fuerza <= 0 || p->destreza <= 0 ||
      p->inteligencia <= 0 || p->fuerza > 999 || p->destreza > 999 ||
      p->inteligencia > 999) {
...
bool es_pokemon_valido(const struct pokemon *p) {
  ...
  // Validar que las estadísticas sean válidas (ID, fuerza, destreza e
  // inteligencia mayores a 0)
  if (p->id <= 0 || p->fuerza <= 0 || p->destreza <= 0 ||
      p->inteligencia <= 0) {
    fprintf(stderr,
            "Advertencia: Pokémon inválido con ID %u (estadísticas negativas o "
            "cero).\n",
            p->id);
    return false;
  }
  ...
  if (p->fuerza > 999 || p->destreza > 999 || p->inteligencia > 999) {
    fprintf(
        stderr,
        "Advertencia: Pokémon con ID %u tiene estadísticas fuera de rango.\n",
        p->id);
    return false;
  }
...
static bool pokedex_agregar_pokemon(pokedex_t *pokedex,
                                    struct pokemon pokemon) {
  if (pokedex->cantidad >= pokedex->capacidad) {
    size_t nueva_capacidad = pokedex->capacidad * 2;
    struct pokemon *nueva_lista =
        realloc(pokedex->pokemones, nueva_capacidad * sizeof(struct pokemon));
    if (!nueva_lista)
      return false;
...
pokedex_t *pokedex_abrir(const char *archivo) {
  ...
  while (fgets(linea, sizeof(linea), f)) {
    ...
    if (separadores < 5) {
      fprintf(stderr,
              "Advertencia: Línea con campos insuficientes descartada: %s\n",
              linea);
...
unsigned pokedex_iterar_pokemones(pokedex_t *pokedex, enum modo_iteracion modo,
  ...
  for (size_t i = 0; i < pokedex->cantidad; i++) {
    for (size_t j = i + 1; j < pokedex->cantidad; j++) {
      if ((modo == ITERAR_NOMBRE && strcmp(pokedex->pokemones[i].nombre,
                                           pokedex->pokemones[j].nombre) > 0) ||
          (modo == ITERAR_ID &&
           pokedex->pokemones[i].id > pokedex->pokemones[j].id)) {
